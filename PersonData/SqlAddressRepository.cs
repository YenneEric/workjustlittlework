   ...
   public class SqlAddressRepository : IAddressRepository
   {
      ...
      private IReadOnlyList<Address> TranslateAddresses(SqlDataReader reader)
      {
         ...
         var zipCodeOrdinal = reader.GetOrdinal("ZipCode");
            ...
            var addresstyp = new AddressType(1, "frank");
        ...
        public IReadOnlyList<string> GetAddressName()
        {
            try
            {
                using (var connection = new SqlConnection(connectionString))
                {
                    using (var command = new SqlCommand("Person.GetAddressName", connection))
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        connection.Open();
                        ...
                        using (var reader = command.ExecuteReader())
                        {
                            var addressTypeNames = new List<string>();

                            while (reader.Read())
                            {
                                addressTypeNames.Add(reader.GetString(reader.GetOrdinal("Name")));
                            }
            ...
            {
                // Log error (adjust logging mechanism to your needs)
                Console.WriteLine($"HAHAError retrieving Address Type Names: {ex.Message}");
                throw;
            }
        }
